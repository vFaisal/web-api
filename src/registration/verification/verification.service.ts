...
export class VerificationService {
  ...
  public async createEmailVerification(email: string) {
    ...
    await this.kv.setex(
      `otp:${signature}`,
      VerificationService.ONE_TIME_PASSWORD_EXPIRATION,
      new OneTimePasswordEntity<'CREATION'>({
        phoneOrEmail: email,
        target: 'EMAIL',
        intent: 'REGISTRATION',
        signature,
  ...
  public async verifyEmail({
    ...
    let oneTimePassword = new OneTimePasswordEntity<'GET'>(
      await this.kv.get(`otp:${signature}`),
    );

    if (
      !oneTimePassword.isValid() ||
      oneTimePassword.phoneOrEmail !== email ||
      oneTimePassword.target !== 'EMAIL'
    )
    ...
    if (
      oneTimePassword.attempts >= VerificationService.DEFAULT_ALLOWED_ATTEMPTS
    )
    ...
    if (!codeVerify) {
      await this.kv.setex(
        `otp:${signature}`,
        VerificationService.ONE_TIME_PASSWORD_EXPIRATION -
          (unixTimestamp() - oneTimePassword.createdTimestampAt),
        oneTimePassword,
      );
      throw new BadRequestException({
    ...
    await this.kv.setex(
      ...
      new RegistrationEntity<'CREATION'>({
