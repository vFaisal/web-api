...
export default class TrustedAccountSessionEntity {
  ...
  constructor(session: Partial<AccountSession & {
    tokens: (Partial<AccountSessionTokens> & { visitor: Partial<Visitor> })[]
    ...
    this.activity = session.tokens.map(t => {
      ...
      return ({
        ...
        counterCode: t.visitor.counterCode?.length === 2 ? t.visitor.counterCode : null,
        location: null,
        region: null,
        ip: t.visitor.ipAddress && isIP(t.visitor.ipAddress) ? t.visitor.ipAddress : null,
        createdAt: Math.round(t.createdAt.getTime() / 1000)
      });
    });
    this.isCurrentSession = !!session.tokens.find(t => t.token === currentSessionToken);
    this.lastSeen = this.activity.at(-1).createdAt;
    this.createdAt = Math.round(session.createdAt.getTime() / 1000);
  }

  @Expose()
  get suspiciousLevel(): number { // Max 5
    return 0;
  }
