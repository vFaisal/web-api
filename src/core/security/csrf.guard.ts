...
export default class CSRFGuard implements CanActivate {
  ...
  public async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!token || !secret)
      throw new BadRequestException({
        code: 'csrf_required',
        message:
          'This endpoint requires a CSRF policy to be included in the request. Please provide a valid policy and try again.',
      });

    if (!this.csrfService.dep.verify(secret, token))
      throw new BadRequestException({
        code: 'invalid_csrf',
        message:
  ...
  private extractSecretFromCookie(
