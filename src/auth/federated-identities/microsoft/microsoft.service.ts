...
export class MicrosoftService {
  ...
  private static readonly REDIRECT_URI =
    'https://api.faisal.gg/v1/auth/federated-identities/microsoft/callback';
  ...
  public async exchangeAuthorizationCode(code: string, codeVerifier: string) {
    ...
    if (!res.ok) {
      ...
      this.logger.error(
        "unexpected response from oauth2 Microsoft api 'login.microsoftonline.com/consumers/oauth2/v2.0/token'",
        data,
      );
    ...
    if (
      !MicrosoftService.APPLICATION_SCOPES.every((s) => scopes.includes(s)) ||
      !scopes.every((s) => MicrosoftService.APPLICATION_SCOPES.includes(s))
    )
      throw new BadRequestException({
  ...
  private async getProfilePhoto(accessToken: string): Promise<string | null> {
    const res = await fetch(
      MicrosoftService.API_BASE + '/me/photos/504x504/$value',
      {
        headers: {
    ...
    if (res.ok) {
      ...
      if (photoBuffer)
        return `data:image/jpeg;base64, ${Buffer.from(photoBuffer).toString(
          'base64',
        )}`;
    }
    return null;
