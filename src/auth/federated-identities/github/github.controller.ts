...
export class GithubController {
  constructor(private readonly githubService: GithubService, private readonly csrfService: CSRFService) {
  }
  ...
  callback(@Query() query: FederatedIdentityQueryDto, @Req() req) {
    return this.githubService.authenticate(query.code, significantRequestInformation(req));
  }
}
