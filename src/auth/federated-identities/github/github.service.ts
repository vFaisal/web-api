...
export class GithubService {
  ...
  public async authenticate(code: string, significantRequestInformation: SignificantRequestInformation) {
    ...
    if (!userPrimaryEmail?.verified) throw new BadRequestException({
      code: "github_unverified_email",
      message: "The user's Github account email address is not verified and cannot be used for authentication."
    });

    return this.federatedIdentitiesService.authenticate(userPrimaryEmail.email, String(user.id), Provider.MICROSOFT, user.avatar_url, user.name, significantRequestInformation);
  }
  ...
  public async exchangeAuthorizationCode(code: string) {
    ...
    if (!res.ok || data.error) {
      if (data?.error === "bad_verification_code") throw  new BadRequestException({
    ...
    if (!GithubService.APPLICATION_SCOPES.every(s => scopes.includes(s)) || !scopes.every(s => GithubService.APPLICATION_SCOPES.includes(s))) throw new BadRequestException({
  ...
  private async getUser(accessToken: string) {
    ...
    const data = await res.json();
    if (!res.ok) {
      this.logger.error("unexpected response from github api 'api.github.com/user'", data);
      throw new ServiceUnavailableException();
    }
  ...
  private async getUserEmails(accessToken: string) {
    ...
    const data = await res.json();
    if (!res.ok) {
      this.logger.error("unexpected response from github api 'api.github.com/user/emails'", data);
      throw new ServiceUnavailableException();
    }
