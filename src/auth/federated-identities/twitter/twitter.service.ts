...
export class TwitterService {
  ...
  private static readonly REDIRECT_URI =
    'https://api.faisal.gg/v1/auth/federated-identities/twitter/callback';
  ...
  public async exchangeAuthorizationCode(code: string, codeVerifier: string) {
        ...
        authorization:
          ...
          Buffer.from(
            `${this.config.getOrThrow(
              'TWITTER_CLIENT_ID',
            )}:${this.config.getOrThrow('TWITTER_CLIENT_SECRET')}`,
          ).toString('base64'),
      },
    ...
    if (!res.ok) {
      ...
      this.logger.error(
        "unexpected response from oauth2 Twitter api 'api.twitter.com/2/oauth2/token'",
        data,
      );
    ...
    if (
      !TwitterService.APPLICATION_SCOPES.every((s) => scopes.includes(s)) ||
      !scopes.every((s) => TwitterService.APPLICATION_SCOPES.includes(s))
    )
      throw new BadRequestException({
