...
import { SignificantRequestInformation, unixTimestamp } from "../../../utils/util";
...
export class FacebookService {
  ...
  private static readonly AUTH_ENDPOINT = "https://www.facebook.com/v17.0/dialog/oauth";
  private static readonly APPLICATION_SCOPES = ["openid", "public_profile", "email"];
  private static readonly REDIRECT_URI = "https://api.faisal.gg/v1/auth/federated-identities/facebook/callback";
  ...
  public async authenticate(code: string, codeVerifier: string, significantRequestInformation: SignificantRequestInformation) {
    ...
    const picture = (await this.getProfilePicture(auth.access_token)) ?? userInfo.picture;

    return this.federatedIdentitiesService.authenticate(userInfo.email, userInfo.sub, Provider.META, picture, userInfo.name, significantRequestInformation);
  }
  ...
  public async exchangeAuthorizationCode(code: string, codeVerifier: string) {
    ...
    const res = await fetch(FacebookService.API_BASE + "/oauth/access_token?" + params.toString());

    const data = await res.json();
    if (!res.ok) {
      if (data.error.code == 100) throw  new BadRequestException({
        code: "facebook_invalid_grant",
        message: "The authorization code is invalid or has expired."
      });
      this.logger.error("unexpected response from oauth2 Facebook api 'graph.facebook.com/v17.0/oauth/access_token'", data);
  ...
  private async getProfilePicture(accessToken) {
    ...
    const params = new URLSearchParams({
      ...
      appsecret_proof: createHmac("sha256", this.config.getOrThrow("FACEBOOK_CLIENT_SECRET")).update(accessToken + "|" + timestamp).digest("hex"),
