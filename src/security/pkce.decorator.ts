...
export const PkceVerifierCode = createParamDecorator(
  (data: unknown, ctx: ExecutionContext) => {
    ...
    if (!unsignedCodeVerifier?.valid) throw new BadRequestException({
      code: "pkce_required",
      message: "The PKCE code verifier is missing or has not been signed in the cookie. Please ensure that a valid code verifier is stored in the cookie."
    });
